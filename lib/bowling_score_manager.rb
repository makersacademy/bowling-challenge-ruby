...
#    (frames_array[frame_num]).roll1 = rollsArray[roll_from_start]
    ...
    if ((frame_num > 1) && frames_array[frame_num-1].status == :spare)
      ...
      frames_array[frame_num-1].total += (frames_array[frame_num]).roll1
      frames_array[frame_num-1].completed = true
    end
    # Deal with possible strike from previous frame
    if ((frame_num > 1) && frames_array[frame_num-1].status == :strike)
      ...
      if ((frame_num > 2) && frames_array[frame_num-2].status == :strike)
        # Add this roll to strike from two frames ago
        # and mark it complete as both of its following rolls
        # have now been added
        frames_array[frame_num-2].total += (frames_array[frame_num]).roll1
        frames_array[frame_num-1].completed = true
      end
    end
    # Mark possible strike in this frame
    if (frames_array[frame_num]).roll1 == 10
      frames_array[frame_num].status = :strike
      frames_array[frame_num].roll2 = 0
      frames_array[frame_num].total = 10
      frames_array[frame_num].completed = false
