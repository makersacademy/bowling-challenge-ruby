...
RSpec.describe Frame do
  context "given a full 2 rolls" do
    it "adds the pins to frame points" do
      frame_one = Frame.new(5, 3, 0)
      expect(frame_one.frame_points).to eq([5, 3, 0])
    end
  end

  context "given first roll is 10" do
    it "skips the next roll" do
      frame_one = Frame.new(10, 3, 0)
      expect(frame_one.frame_points).to eq([10, 3, 0])
    end
  end
