...
describe ScoreCalculator do
  subject(:scorecalculator) { described_class.new }
  ...
  describe '#frame_scores' do
    context 'when less than 10 frames played' do
      it 'returns score for one non-strike/spare frame' do
        frames = [[3, 4]]
        scores = [7]
        expect(scorecalculator.frame_scores(frames)).to eq(scores)
      end
    ...
    context 'when 10 frames played' do
      ...
      it 'returns score for non-strike/spare in 10th frame' do
        ...
        tenth_frame_score = 5
        expect(scorecalculator.frame_scores(frames)[tenth_index]).to eq(tenth_frame_score)
      end
      ...
      it 'returns score for strike in 9th frame where extra rolls in tenth frame' do
        ...
        expect(scorecalculator.frame_scores(frames)[ninth_index]).to eq(ninth_frame_score)
      end
  ...
  describe '#total score' do
    it 'returns total score for a gutter game' do
      ...
      10.times { frames << [0, 0] }
      score = 0
      expect(scorecalculator.game_score(frames)).to eq(score)
    end
