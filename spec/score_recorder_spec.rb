...
describe ScoreRecorder do
  ...
  describe '#add_roll' do
    it 'adds first roll' do
      score_recorder = described_class.new
    ...
    it 'adds second roll' do
      frames = [[1]]
      score_recorder = described_class.new(frames)
    ...
    it 'does not add roll after complete game (no extra roll)' do
      frames = nine_frames << [7, 1]
      score_recorder = described_class.new(frames)
      score_recorder.add_roll(6)
      expected_frames = nine_frames << [7, 1]
      expect(score_recorder.frames).to eq(expected_frames)
    end

    it 'does not add roll after extra roll' do
      frames = nine_frames << [10, 10, 10]
      score_recorder = described_class.new(frames)
      score_recorder.add_roll(10)
      expected_frames = nine_frames << [10, 10, 10]
      expect(score_recorder.frames).to eq(expected_frames)
    end
  end
